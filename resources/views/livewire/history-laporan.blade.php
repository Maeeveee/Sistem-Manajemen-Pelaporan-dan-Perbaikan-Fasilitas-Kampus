            ...
            <div class="col-md-6 mb-3 mb-md-0">
                <div class="input-group">
                    ...
                    <input type="text" class="form-control"
                        placeholder="Cari berdasarkan gedung, ruangan, fasilitas..." wire:model.live="search">
                </div>
            </div>
            <div class="col-md-6">
                <div class="input-group">
                            ...
                            <div class="col-md-12">
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control bg-white"
                                        value="{{ $selectedLaporan->created_at->format('Y-m-d') }}" readonly>
                                </div>

                                <div class="mb-3">
                                    <label class="form-label">Nama Pelapor</label>
                                    <input type="text" class="form-control bg-white"
                                        value="{{ $selectedLaporan->nama_pelapor ?? '-' }}" readonly>
                                </div>

                                <div class="mb-3">
                                    <label class="form-label">NIM/NIP</label>
                                    <input type="text" class="form-control bg-white"
                                        value="{{ $selectedLaporan->identifier ?? '-' }}" readonly>
                                </div>

                                <div class="mb-3">
                                    <label class="form-label">Gedung</label>
                                    <input type="text" class="form-control bg-white"
                                        value="{{ $selectedLaporan->gedung->nama_gedung ?? '-' }}" readonly>
                                </div>

                                <div class="mb-3">
                                    <label class="form-label">Ruangan</label>
                                    <input type="text" class="form-control bg-white"
                                        value="{{ $selectedLaporan->ruangan->nama_ruangan ?? '-' }}" readonly>
                                </div>

                                <div class="mb-3">
                                    <label class="form-label">Lantai</label>
                                    <input type="text" class="form-control bg-white"
                                        value="{{ $selectedLaporan->lantai }}" readonly>
                                </div>

                                <div class="mb-3">
                                    <label class="form-label">Fasilitas</label>
                                    <input type="text" class="form-control bg-white"
                                        value="{{ $selectedLaporan->fasilitas->nama_fasilitas ?? '-' }}" readonly>
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control bg-white"
                                        value="{{ ucfirst($selectedLaporan->status_admin) }}" readonly>
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    @if ($selectedLaporan->foto)
                                        <img src="{{ asset('storage/' . $selectedLaporan->foto) }}"
                                            alt="Foto Kerusakan" class="img-thumbnail" style="max-width: 300px;">
                                    @else
                                ...
                                @if ($selectedLaporan->status === 'selesai' && is_null($selectedLaporan->rating))
                                    <div class="mb-3">
                                        <label class="form-label">Beri Rating (1-5)</label>
                                        <div class="star-rating" style="font-size:2rem;">
                                            @for ($i = 1; $i <= 5; $i++)
                                                <span class="star {{ $rating >= $i ? 'text-secondary' : 'text-warning' }}"
                                                    style="cursor:pointer;"
                                                    wire:click="$set('rating', {{ $i }})"
                                                    wire:key="star-{{ $i }}">&#9733;</span>
                                            @endfor
                                        </div>
                                        @error('rating')
                                    ...
                                    @if ($rating)
                                        <div class="mb-3">
                                            ...
                                            <textarea class="form-control bg-white" rows="3" wire:model.defer="feedback" placeholder="Tulis ulasan Anda di sini..."></textarea>
                                        </div>
                                        <div class="mb-3">
                                            <button class="btn btn-success" wire:click="submitRating">Kirim Rating</button>
                                        </div>
                                    @endif
                                @elseif(!is_null($selectedLaporan->rating))
                                    <div class="mb-3">
                                        <label class="form-label">Rating Anda</label>
                                        <div class="star-rating" style="font-size:2rem;">
                                            @for ($i = 1; $i <= 5; $i++)
                                                <span class="star {{ $selectedLaporan->rating >= $i ? 'text-secondary' : 'text-warning' }}">&#9733;</span>
                                            @endfor
                                        </div>
                                    </div>
                                    <div class="mb-3">
                                        <label class="form-label">Ulasan anda</label>
                                        <textarea class="form-control bg-white" rows="3" readonly>{{ $selectedLaporan->feedback ?? '-' }}</textarea>
                                    </div>
                                @endif
                            </div>
