        ...
        <div class="mb-4">
            ...
            <select wire:model="periodeId" class="form-control">
                ...
                @foreach ($daftarPeriode as $periode)
                    <option value="{{ $periode->id }}">{{ $periode->nama_periode }} ({{ $periode->tanggal_mulai }} -
                        {{ $periode->tanggal_selesai }})</option>
                @endforeach
            </select>
            @error('periodeId')
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <!-- filepath: resources/views/livewire/perhitungan-spk.blade.php -->

                    <!-- Update tabel rekomendasi prioritas -->
                        ...
                        <table class="table table-bordered table-hover">
                            ...
                            <tbody>
                                @if (empty($sortedResults))
                                    <tr>
                                        <td colspan="8" class="text-center">Data akan muncul setelah memilih periode
                                        </td>
                                    </tr>
                                @else
                                    @foreach ($sortedResults as $item)
                                        <tr>
                                            ...
                                            <td>
                                                <span class="fw-bold">{{ number_format($item['nilai'], 3) }}</span>
                                            </td>
                                            <td>
                                                <span>{{ $item['total_laporan'] }}</span>
                                            </td>
                                            <td>
                                                @php
                                                    $laporanId = $laporan[$item['original_index']]['id'];
                                                    $laporanDetail = \App\Models\LaporanKerusakan::find($laporanId);
                                                @endphp

                                                @if ($laporanDetail)
                                                    @switch($laporanDetail->status_perbaikan)
                                                        @case('menunggu')
                                            ...
                                            <td>
                                                @if ($laporanDetail && $laporanDetail->teknisi)
                                                    <small
                                                        class="text-muted">{{ $laporanDetail->teknisi->name }}</small>
                                                @else
                                            ...
                                            <td>
                                                @php
                                                    $isProcessed = $this->isLaporanSudahDiproses(
                                                        $laporan[$item['original_index']]['id'],
                                                    );
                                                @endphp
                                                ...
                                                @else
                                                    <button
                                                        wire:click="openProsesModal({{ $laporan[$item['original_index']]['id'] }})"
                                                        class="btn btn-sm btn-primary" title="Proses laporan ini">
                                                        <i class="fas fa-cog"></i> Proses
                                                    </button>
                                                @endif

                                                <button
                                                    wire:click="openDetailModal({{ $laporan[$item['original_index']]['id'] }})"
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-hover align-middle text-center">
                            ...
                            <tbody>
                                @if (empty($laporan))
                                    <tr>
                                        <td colspan="12" class="text-center">Data akan muncul setelah memilih periode
                                        </td>
                                    </tr>
                                @else
                            ...
                            <tfoot>
                                <tr class="table-info">
                                    <td colspan="5" class="text-center align-middle"><strong>Bobot Kriteria
                                            (W)</strong></td>
                                ...
                                <tr class="table-info">
                                    <td colspan="5" class="text-center align-middle"><strong>Atribut
                                            Kriteria</strong></td>
                                    <td>Benefit</td>
                                    <td>Benefit</td>
                                    <td>Cost</td>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-hover">
                            ...
                            <tbody>
                                @if (empty($normalized))
                                    <tr>
                                        <td colspan="7" class="text-center">Data akan muncul setelah memilih periode
                                        </td>
                                    </tr>
                                @else
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-hover">
                            ...
                            <tbody>
                                @if (empty($results))
                                    <tr>
                                        <td colspan="5" class="text-center">Data akan muncul setelah memilih periode
                                        </td>
                                    </tr>
                                @else
                    ...
                    <div class="modal-body">
                        <form wire:submit.prevent="prosesLaporan">
                            ...
                            <div class="form-group mb-3">
                                ...
                                <select class="form-control" id="statusPerbaikan" wire:model="statusPerbaikan"
                                    required>
                                    <option value="">-- Pilih Status --</option>
                                    <option value="diproses">Ditugaskan</option>
                                </select>
                                @error('statusPerbaikan')
                                    <small class="text-danger">{{ $message }}</small>
                                @enderror
                            </div>
                            <div class="form-group">
                                            ...
                                            @if ($hasilSpk)
                                                <table class="table table-sm">
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <span
                                                                class="badge bg-{{ $hasilSpk->nilai > 0.7 ? 'danger' : ($hasilSpk->nilai > 0.5 ? 'warning' : 'success') }}">
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>{{ $laporanDetail->frekuensiSubKriteria->nama_subkriteria ?? '-' }}
                                                        </td>
                                                    </tr>
                                                    <tr>
                                                        <th>Dampak Akademik</th>
                                                        <td>{{ $laporanDetail->dampakSubKriteria->nama_subkriteria ?? '-' }}
                                                        </td>
                                                    </tr>
                                                    <tr>
                                                        <th>Tingkat Resiko</th>
                                                        <td>{{ $laporanDetail->resikoSubKriteria->nama_subkriteria ?? '-' }}
                                                        </td>
                                                    </tr>
                                                    <tr>
                                                        <th>Tingkat Kerusakan</th>
                                                        <td>{{ $laporanDetail->kerusakanSubKriteria->nama_subkriteria ?? '-' }}
                                                        </td>
                                                    </tr>
                                                    <tr>
                                                        <th>Estimasi Waktu</th>
                                                        <td>{{ $laporanDetail->subKriteria->nama_subkriteria ?? '-' }}
                                                        </td>
                                                    </tr>
                                                </table>
                                            @else
